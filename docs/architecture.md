...
| Fibrin | Zero | Hemostasis platelet activation, fibrin mesh scaffold | Clot forms in wound |
| pH | Dermal neutral (7.4) | Wound acidification from lactate/inflammation | None (starts neutral) |
| Temperature | Body core (37C) | Surface cooling at wound, Q10 enzymatic rate scaling | Wound surface drops to 35C |
| Glucose | Dermal supply | Perfusion-gated delivery, metabolic consumption | Zero in wound dermis |
| Lactate | Zero | Hypoxia-driven production, perfusion clearance | None (starts at zero) |
| NitricOxide | Zero | iNOS from macrophages, eNOS from endothelium | None (starts at zero) |
...
- **O2-modulated proliferation**: G1 to S transition in `BasalDivision` scales with local O2, so margin cells (near intact vasculature) divide faster than wound-center cells.
- **Water-gated migration and proliferation**: tissue moisture gates both crawling speed (in `Migration`) and G1 to S probability (in `BasalDivision`). After wounding, the wound bed must re-hydrate from dermal serum before effective re-epithelialization can begin.
- **Inflammation-gated migration and proliferation**: immune cell agents write pro-inflammatory cytokines into the Inflammation field with an age-dependent taper (neutrophils: `exp(-0.005 * age)`, M1 macrophages: `exp(-0.003 * state_age)`). High inflammation suppresses keratinocyte crawling and G1 to S transition. Macrophages transition M1 to M2, actively consuming cytokines to resolve inflammation after ~3 to 5 days.
- **TIMP-mediated MMP regulation**: TIMP (tissue inhibitor of metalloproteinases) is produced by fibroblasts, M2 macrophages, and wound-edge keratinocytes. Second-order MMP*TIMP neutralization at 1:1 stoichiometry provides mechanistic MMP clearance alongside residual endocytic decay.
