...
The schedule generator creates scenario TOMLs that model delayed treatment start. A treatment started at day 0 has full effect; later starts have proportionally reduced effect based on the remaining healing window (42 days default).
...
Output goes to `studies/diabetic-wound/scenarios/schedule_*.toml`. Each scenario includes an untreated diabetic baseline and configs for each start day, with parameters interpolated between the diabetic profile and full treatment values.
