 ...
 private:
  ...
  void SpawnCells(Simulation* sim, const SimParam* sp,
    ...
    for (int i = 0; i < n_cells; i++) {
      ...
      // wound interior (random position within wound radius), not at
      // the perimeter where keratinocytes re-epithelialize.
      real_t rand_r = r * std::sqrt(random->Uniform(0, 1));
      real_t theta = random->Uniform(0, 2.0 * M_PI);
      real_t x = cx + rand_r * std::cos(theta);
      real_t y = cy + rand_r * std::sin(theta);
