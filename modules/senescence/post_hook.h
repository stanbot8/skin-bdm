...
// Immune-dependent clearance: NK cells and M2 macrophages selectively eliminate
// senescent cells via NKG2D ligand recognition and efferocytosis.
// Demaria et al. 2014 (doi:10.1016/j.devcel.2014.11.012)
// Coppe et al. 2008 (doi:10.1371/journal.pbio.0060301)
// Sagiv et al. 2016 (doi:10.18632/aging.100897): NK surveillance of senescence
struct SenescencePostHook {
  ...
  DiffusionGrid* ip_grid = nullptr;  // immune pressure (NK + macrophage density proxy)
  ...
  inline void Init(const GridRegistry& reg, SignalBoard& sig) {
    ...
    ip_grid = reg.Get(fields::kImmunePressureId);
  ...
  inline void ApplyEpiWound(const VoxelSnapshot& snap, SignalBoard& sig) {
    ...
    real_t wound_gate = snap.WoundGate();
    ...
    // Immune-mediated senescence clearance (NK cells + M2 macrophages).
    // NK cells recognize NKG2D ligands (MICA/MICB) upregulated on senescent
    // cells. M2 macrophages perform efferocytosis of senescent cells.
    // Clearance rate scales with local immune pressure as a proxy for
    // NK/macrophage density. Sagiv et al. 2016; Kang et al. 2011.
    real_t sen_val = sen_grid->GetConcentration(sen_si);
    if (ip_grid && sp_->senescence.immune_clearance_rate > 0 && sen_val > 1e-10) {
      real_t ip_val = ip_grid->GetConcentration(snap.idx);
      if (ip_val > 1e-10) {
        accum -= sp_->senescence.immune_clearance_rate * sen_val * ip_val;
      }
    }
