...
struct DiabeticParams {
  ...
  real_t baseline_inflammation = 0.0005;  // per-step inflammation added to wound
  real_t neutrophil_factor = 1.8;  // neutrophil spawn count multiplier
  real_t neutrophil_lifespan_factor = 3.0;  // neutrophil lifespan multiplier
  real_t neutrophil_apoptosis_factor = 0.3;  // impaired neutrophil apoptosis
  ...
  real_t migration_infl_K = 0.08;  // Hill half-max for migration inflammation gating (vs 1.0 normal)
  real_t prolif_infl_K = 0.12;  // Hill half-max for proliferation inflammation gating (vs 1.5 normal)
