...
inline void WriteScarValue(Keratinocyte* cell) {
  ...
  // Stratum field: differentiated scar cells get stratum+5 (scar strata 6-8
  // = scar spinosum/granulosum/corneum). Basal cells (stratum=0) must NOT
  // receive the +5 offset -- that would write 5.0 into empty voxels and
  // artificially inflate wound coverage (ComputeCoverage checks > 0.5).
  // A basal cell still migrating at dissolution has not re-epithelialized
  // its voxel; only differentiated cells (spinosum+) represent healed tissue.
  ...
  if (stratum_grid) {
    ...
    if (is_scar && new_val >= 1.0) new_val += 5.0;
