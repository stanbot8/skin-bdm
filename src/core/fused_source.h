...
struct FusedWoundSourceOp : public StandaloneOperationImpl {
  ...
  void operator()() override {
    ...
    if (do_temp) {
      ...
      if (!temp_grid) do_temp = false;
    }
    DiffusionGrid* glucose_grid = nullptr;
    bool do_glucose = sp->glucose_enabled;
    if (do_glucose) {
      glucose_grid = rm->GetDiffusionGrid(fields::kGlucoseId);
      if (!glucose_grid) do_glucose = false;
    }
    DiffusionGrid* lactate_grid = nullptr;
    bool do_lactate = sp->lactate_enabled;
    if (do_lactate) {
      lactate_grid = rm->GetDiffusionGrid(fields::kLactateId);
      if (!lactate_grid) do_lactate = false;
    }
    DiffusionGrid* no_grid = nullptr;
    bool do_no = sp->nitric_oxide_enabled;
    if (do_no) {
      no_grid = rm->GetDiffusionGrid(fields::kNitricOxideId);
      if (!no_grid) do_no = false;
    }

    // Nerve regeneration grid pointers
    DiffusionGrid* nerve_grid = nullptr;
    DiffusionGrid* vegf_nerve = nullptr;
    DiffusionGrid* tgfb_nerve = nullptr;
    bool do_nerve_regen = sp->neuropathy_enabled;
    if (do_nerve_regen) {
      nerve_grid = rm->GetDiffusionGrid(fields::kNerveId);
      if (!nerve_grid) do_nerve_regen = false;
      if (do_nerve_regen && sp->angiogenesis_enabled)
        vegf_nerve = rm->GetDiffusionGrid(fields::kVEGFId);
      if (do_nerve_regen && sp->fibroblast_enabled)
        tgfb_nerve = rm->GetDiffusionGrid(fields::kTGFBetaId);
    }
    ...
    if (do_dermis) {
      ...
      if (!dermis_grid) do_dermis = false;
      if (do_dermis && sp->fibroblast_enabled)
        col_grid_dermis = rm->GetDiffusionGrid(fields::kCollagenId);
      if (do_dermis && sp->mmp_enabled)
        mmp_grid_dermis = rm->GetDiffusionGrid(fields::kMMPId);
    }
    ...
    for (size_t idx : mask_.dermal_all) {
      ...
      // -- Temperature: perfusion warms wound tissue toward body temp --
      if (do_temp && in_wound) {
        real_t temp_cur = temp_grid->GetConcentration(idx);
        if (temp_cur < 1.0 - 1e-10) {
          real_t warm = sp->temperature_perfusion_warming * vasc_val;
          real_t gain = std::min(1.0 - temp_cur, warm);
          if (gain > 1e-10) {
            temp_grid->ChangeConcentrationBy(idx, gain);
          }
        }
      }

      // -- Glucose: perfusion supplies glucose to dermal voxels --
      if (do_glucose) {
        real_t gluc_cur = glucose_grid->GetConcentration(idx);
        real_t gluc_target = sp->glucose_basal_conc * vasc_val;
        if (gluc_cur < gluc_target - 1e-10) {
          real_t gain = std::min(gluc_target - gluc_cur,
                                 sp->glucose_perfusion_supply);
          if (gain > 1e-10) {
            glucose_grid->ChangeConcentrationBy(idx, gain);
          }
        }
      }

      // -- Lactate: hypoxia-driven production + perfusion clearance --
      if (do_lactate && in_wound && post_wound) {
        real_t local_o2 = o2_grid->GetConcentration(idx);
        if (local_o2 < sp->lactate_o2_threshold) {
          real_t prod = sp->lactate_production_rate *
                        (sp->lactate_o2_threshold - local_o2) /
                        sp->lactate_o2_threshold;
          if (prod > 1e-10) {
            lactate_grid->ChangeConcentrationBy(idx, prod);
          }
        }
        // Perfusion clears lactate (washout)
        real_t lac_cur = lactate_grid->GetConcentration(idx);
        if (lac_cur > 1e-10 && vasc_val > 0.1) {
          real_t clear = std::min(lac_cur,
                                   sp->lactate_perfusion_clearance * vasc_val);
          lactate_grid->ChangeConcentrationBy(idx, -clear);
        }
      }

      // -- NO vasodilation: boost effective perfusion target --
      if (do_no && in_wound && vasc_eligible) {
        real_t no_val = no_grid->GetConcentration(idx);
        if (no_val > 1e-10 && vasc_val < local_vasc_target) {
          real_t no_boost = vasc_rate * sp->no_vasodilation_factor * no_val;
      ...
      // -- Nerve regeneration (wound dermal only) --
      ...
      if (do_nerve_regen && in_wound && post_wound) {
        size_t n_si = sidx(nerve_grid, idx);
        real_t nerve_val = nerve_grid->GetConcentration(n_si);
        real_t nerve_target = sp->neuropathy_basal_density;
        if (sp->diabetic_mode) {
          nerve_target *= sp->diabetic_nerve_factor;
        }
        if (nerve_val < nerve_target - 1e-10) {
          real_t rate = sp->nerve_regeneration_rate;
          if (sp->diabetic_mode) {
            rate *= sp->diabetic_nerve_regen_factor;
          }
          // VEGF neurovascular coupling: promotes nerve sprouting
          if (vegf_nerve) {
            real_t local_vegf = vegf_nerve->GetConcentration(idx);
            rate *= (1.0 + sp->nerve_vegf_boost * local_vegf);
          }
          // TGF-beta scar barrier: inhibits neurite extension
          if (tgfb_nerve) {
            real_t local_tgfb = tgfb_nerve->GetConcentration(idx);
            rate *= std::max(static_cast<real_t>(0),
                             1.0 - sp->nerve_tgfb_inhibit * local_tgfb);
          }
          real_t gain = std::min(nerve_target - nerve_val, rate);
          if (gain > 1e-10) {
            nerve_grid->ChangeConcentrationBy(n_si, gain * coarse_w);
          }
        }
      }
    }
    ...
    for (size_t idx : mask_.epi_wound) {
      ...
      if (do_wound_infl && stratum_val < 1.0) {
        ...
        real_t s_clamped = std::max(static_cast<real_t>(0.0), stratum_val);
      ...
      // -- Temperature: exposed wound surface cools --
      if (do_temp) {
        real_t temp_cur = temp_grid->GetConcentration(idx);
        real_t temp_target = sp->temperature_wound_surface;
        // Cooling proportional to exposed surface (1 - barrier)
        real_t expose = 1.0 - barrier;
        if (expose > 1e-10 && temp_cur > temp_target + 1e-10) {
          real_t cool = sp->temperature_surface_cooling * expose;
          real_t loss = std::min(temp_cur - temp_target, cool);
          temp_grid->ChangeConcentrationBy(idx, -loss);
        }
        // Perfusion warming from below
        Real3 below_t = {ctx.X(idx), ctx.Y(idx), -1.0};
        real_t perf_below = vasc_grid->GetValue(below_t);
        if (temp_cur < 1.0 - 1e-10 && perf_below > 0.1) {
          real_t warm = sp->temperature_perfusion_warming * perf_below;
          real_t gain = std::min(1.0 - temp_cur, warm);
          if (gain > 1e-10) {
            temp_grid->ChangeConcentrationBy(idx, gain);
          }
      ...
      // -- Lactate: epidermal hypoxia production --
      if (do_lactate && post_wound) {
        real_t local_o2 = o2_grid->GetConcentration(idx);
        if (local_o2 < sp->lactate_o2_threshold) {
          real_t prod = sp->lactate_production_rate *
                        (sp->lactate_o2_threshold - local_o2) /
      ...
      // -- Lactate VEGF boost: lactate stabilizes HIF-1a --
      if (do_lactate && do_vegf && post_wound) {
        real_t lac_val = lactate_grid->GetConcentration(idx);
        if (lac_val > 1e-10) {
          real_t local_o2 = o2_grid->GetConcentration(idx);
          if (local_o2 < vegf_threshold) {
            real_t base_vegf = vegf_prod_rate *
                               (vegf_threshold - local_o2) / vegf_threshold;
            real_t boost = base_vegf * sp->lactate_vegf_boost * lac_val;
            if (boost > 1e-10) {
              vegf_grid->ChangeConcentrationBy(idx, boost);
            }
          }
        }
      }
    ...
    if (sp->hemostasis_enabled) {
      auto* fb_grid = rm->GetDiffusionGrid(fields::kFibrinId);
      // Manual decay
      ...
      if (post_wound) {
        DiffusionGrid* tgfb_target = sp->fibroblast_enabled ?
            rm->GetDiffusionGrid(fields::kTGFBetaId) : nullptr;
