...
inline int Simulate(int argc, const char** argv) {
  ...
  // --- Auto-register conditional visualization ---
  {
    ...
    // Keratinocyte: export stratum_ for LUT coloring (uniform flesh via
  ...
  auto* ref_grid = simulation.GetResourceManager()
      ->GetDiffusionGrid(fields::kOxygenId);
  ...
  // Auto-open metrics CSV in default app (user interactive runs only).
  // Set skin.headless = true in bdm.toml to suppress for batch/AI runs.
  if (sp->metrics_autoopen && !sp->headless && sp->metrics_interval > 0) {
