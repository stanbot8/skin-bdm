...
// VolumeManagerTest
// ---------------------------------------------------------------------------
TEST(VolumeManagerTest, StratumLookup) {
  auto* vm = VolumeManager::Get();
  vm->Init(6.0, 15.0, 25.0, -2.0, -8.0);

  // Papillary dermis: z in [-2, 0)
  EXPECT_EQ(vm->GetStratumAt({15, 15, -1}), kPapillary);
  EXPECT_EQ(vm->GetStratumAt({15, 15, -1.5}), kPapillary);

  // Reticular dermis: z in [-8, -2)
  EXPECT_EQ(vm->GetStratumAt({15, 15, -3}), kReticular);
  EXPECT_EQ(vm->GetStratumAt({15, 15, -7}), kReticular);

  // Hypodermis: z < -8
  EXPECT_EQ(vm->GetStratumAt({15, 15, -9}), kHypodermis);

  // Epidermal strata
  EXPECT_EQ(vm->GetStratumAt({15, 15, 3}), kBasal);
  EXPECT_EQ(vm->GetStratumAt({15, 15, 10}), kSpinous);
  EXPECT_EQ(vm->GetStratumAt({15, 15, 20}), kGranular);
  EXPECT_EQ(vm->GetStratumAt({15, 15, 30}), kCornified);
}
