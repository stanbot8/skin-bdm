...
static Simulation* CreateTestSim(const char* name) {
  ...
  auto set_param = [](Param* param) {
    ...
    sp->wound_center_x = 25;   // center of test domain
    sp->wound_center_y = 25;   // center of test domain
...
static void SanitizeForUnitTest(SimParam* sp) {
  ...
  sp->temperature_enabled = false;
  sp->glucose_enabled = false;
  sp->lactate_enabled = false;
  sp->nitric_oxide_enabled = false;
...
TEST(NeutrophilBehaviorTest, StochasticApoptosis) {
  ...
  auto* sp = const_cast<SimParam*>(sim->GetParam()->Get<SimParam>());
  SanitizeForUnitTest(sp);
  sp->wound_enabled = false;              // disable migration (not under test)
...
TEST(WoundResolutionTest, SafetyTimeout) {
  ...
  auto* sp = const_cast<SimParam*>(sim->GetParam()->Get<SimParam>());
  sp->wound_trigger_step = 0;
  sp->num_steps = 12;  // safety net fires at num_steps - 1 = step 11
  ...
  // Run to safety timeout (step >= num_steps - 20 = 11)
  sim->GetScheduler()->Simulate(11);
...
TEST(MetricsExporterTest, WritesHeaderOnFirstCall) {
  ...
  // Count commas to verify column count (44 commas for 45 columns)
  int commas = 0;
  for (char c : header) {
    if (c == ',') commas++;
  }
