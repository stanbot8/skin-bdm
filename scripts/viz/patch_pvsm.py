...
def create_uniform_agent_lut(root, agent_rgb, ref_lut_proxy):
    """Create a new PVLookupTable with uniform color for agent coloring.

    Clones an existing LUT proxy and replaces its RGBPoints so every
    stratum value (0 through 12) maps to the same color.  This avoids
    z-fighting between overlapping agents at different strata while
    keeping the working LUT pipeline (DiffuseColor is ignored by
    ParaView for BioDynaMo Glyph output).

    Returns the new proxy's id string.
    """
    # Unique id: one past the highest existing proxy id
    max_id = 0
    for proxy in root.iter("Proxy"):
        pid = proxy.get("id", "")
        if pid.isdigit():
    ...
    new_lut = copy.deepcopy(ref_lut_proxy)
    new_lut.set("id", new_id)

    # Build uniform RGBPoints: every value gets the same color
    r, g, b = agent_rgb
    flat = []
    for v in range(13):
        flat.extend([float(v), r, g, b])

    for prop in new_lut.findall("Property"):
        name = prop.get("name")
        if name == "RGBPoints":
            for child in list(prop):
                if child.tag == "Element":
    ...
    # Insert into same parent element as the reference LUT
    parent_map = {child: parent for parent in root.iter() for child in parent}
    parent = parent_map.get(ref_lut_proxy, root)
    parent.append(new_lut)
...
def patch_agent_solid_colors(root, agent_colors):
    ...
    Traces the pipeline: GeometryRepresentation -> Input (Glyph proxy id)
    -> ProxyManagerState Item name to identify the agent type.
    """
    # Build id -> name map from ProxyManagerState Item entries
    id_to_name = {}
    ...
    for proxy in root.iter("Proxy"):
        if proxy.get("type") != "GeometryRepresentation":
            continue
        # Find the Input proxy id (the Glyph filter this representation shows)
        input_id = ""
        for prop in proxy.findall("Property"):
            if prop.get("name") == "Input":
                for sub in prop.findall("Proxy"):
                    input_id = sub.get("value", "")
                    break
        if not input_id:
            continue
        input_name = id_to_name.get(input_id, "")
        if not input_name:
            continue

        for agent_name, rgb in agent_colors.items():
            if agent_name not in input_name:
                continue
            # Only apply solid color if no data array is already selected
            # (patch_agent_representation may have set stratum_ coloring)
            has_data_array = False
            for prop in proxy.findall("Property"):
                if prop.get("name") == "ColorArrayName":
                    for el in prop.findall("Element"):
                        if int(el.get("index", "-1")) == 4 and el.get("value", ""):
                            has_data_array = True
            if has_data_array:
                break
            for prop in proxy.findall("Property"):
                ...
                # Force solid color: prevent ParaView from auto-selecting
                # a data array (e.g. diameter_) for scalar mapping
                if pname == "MapScalars":
                    for el in prop.findall("Element"):
                        el.set("value", "0")
            patched += 1
...
def main():
    ...
    # When agent_color.Keratinocyte is defined, create a dedicated LUT that
    # maps all stratum values to the same flesh color.  This avoids
    # z-fighting between overlapping strata while using the working LUT
    # pipeline (DiffuseColor is ignored by ParaView for Glyph output).
    agent_color_cfg = cfg.get("agent_color", {})
    agent_lut_id = first_lut_id
    keratinocyte_cfg = agent_color_cfg.get("Keratinocyte", {})
    if (isinstance(keratinocyte_cfg, dict) and "color" in keratinocyte_cfg
            and first_lut_proxy is not None):
        agent_lut_id = create_uniform_agent_lut(
            root, keratinocyte_cfg["color"], first_lut_proxy)
        print(f"  Agent LUT: uniform flesh {keratinocyte_cfg['color']}")

    if agent_lut_id:
        patch_agent_representation(root, agent_lut_id)
