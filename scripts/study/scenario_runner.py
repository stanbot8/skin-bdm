...
Usage:
    python3 scripts/study/scenario_runner.py studies/diabetic-wound/scenarios/wound_size.toml
    python3 scripts/study/scenario_runner.py studies/*/scenarios/*.toml
    python3 scripts/study/scenario_runner.py studies/diabetic-wound/scenarios/biofilm_infection.toml --runs=3
"""
...
def load_scenario(path):
    ...
    scenario.setdefault("study", "diabetic-wound")
...
def prepare_scenario_config(cfg, scenario):
    ...
    Merges core config, applies profile and study config from the scenario
    ...
    # Study config: per-config overrides scenario-level
    study = cfg.get("study") or scenario.get("study")
    if study:
        apply_study(study)
...
def run_consensus(label, cfg, scenario, n_runs, output_dir):
    ...
    for i in range(n_runs):
        ...
        # Re-create raw_dir: run_simulation() nukes output/ which may contain it
        os.makedirs(raw_dir, exist_ok=True)
...
def write_summary(scenario, results, output_dir, elapsed):
    ...
    with open(path, "w") as f:
        ...
        f.write(f"Study: {scenario.get('study', 'default')}\n")
...
def run_scenario_file(scenario_path, runs_override=None):
    ...
    # Output directory (outside output/ to avoid run_simulation() cleanup)
    safe_name = name.lower().replace(" ", "_").replace("(", "").replace(")", "")
    output_dir = os.path.join(REPO, "studies", "scenarios", safe_name)
