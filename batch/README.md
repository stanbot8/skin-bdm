...
Output: `batch/results/consensus_<skin>_<study>_<timestamp>/`
...
Save PDE grid state at a specific step, then fork with different treatment parameters to resume from that checkpoint. This avoids re-simulating the shared pre-treatment phase and reduces wall time by approximately 37% for day-7 forks.

```bash
# Save a checkpoint at step 1680 (day 7)
python3 batch/checkpoint.py save --step 1680 --skin diabetic --study diabetic-wound

# Fork from that checkpoint with a treatment overlay
python3 batch/checkpoint.py fork --checkpoint checkpoints/step_1680 \
...
- `SKIBIDY_CKPT_STEP`: step number at which to save
...
batch/
  ...
  checkpoint.py      Checkpoint save/fork CLI
