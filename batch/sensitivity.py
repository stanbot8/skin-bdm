...
def summarize_effects(ee):
    ...
    Includes bootstrap 95% CI on mu* when n >= 4, enabling assessment of
    whether sensitivity rankings are statistically distinguishable.

    Returns dict: param_name -> {outcome -> {mu_star, sigma, mu, n,
                                              mu_star_ci_lo, mu_star_ci_hi}}
    """
    ...
    for pname, outcomes in ee.items():
        ...
        for outcome, values in outcomes.items():
            ...
            # Bootstrap 95% CI on mu* (1000 resamples)
            mu_star_ci_lo = mu_star
            mu_star_ci_hi = mu_star
            if n >= 4:
                n_boot = 1000
                boot_mu_stars = []
                for _ in range(n_boot):
                    sample = [_rng.choice(values) for _ in range(n)]
                    boot_mu_stars.append(sum(abs(v) for v in sample) / n)
                boot_mu_stars.sort()
                mu_star_ci_lo = boot_mu_stars[int(n_boot * 0.025)]
                mu_star_ci_hi = boot_mu_stars[int(n_boot * 0.975)]

            summary[pname][outcome] = {
                "mu_star": mu_star, "sigma": sigma, "mu": mu, "n": n,
                "mu_star_ci_lo": mu_star_ci_lo,
                "mu_star_ci_hi": mu_star_ci_hi}
    return summary
...
def run_sensitivity(cfg, override_r=None, override_levels=None):
    ...
    for outcome in all_outcomes:
        ...
        print(f"  {'Parameter':<40} {'mu*':>8} {'95% CI mu*':>20} {'sigma':>8} {'n':>4}")
        print(f"  {'-' * 82}")
        ...
        for pname, outcomes_data in ranked:
            ...
            sigma = s.get("sigma", 0)
            n = s.get("n", 0)
            ci_lo = s.get("mu_star_ci_lo", mu_star)
            ci_hi = s.get("mu_star_ci_hi", mu_star)
            flag = " ***" if mu_star > 0.5 else " *" if mu_star > 0.1 else ""
            ci_str = f"[{ci_lo:.4f}, {ci_hi:.4f}]"
            short_name = pname.split(".")[-1]
            print(f"  {short_name:<40} {mu_star:>8.4f} {ci_str:>20} {sigma:>8.4f} {n:>4}{flag}")
    ...
    for pname, outcomes_data in summary.items():
        for outcome, s in outcomes_data.items():
            rows.append({
                ...
                "outcome": outcome,
                "mu_star": s["mu_star"],
                "mu_star_ci_lo": s.get("mu_star_ci_lo", s["mu_star"]),
                "mu_star_ci_hi": s.get("mu_star_ci_hi", s["mu_star"]),
                "sigma": s["sigma"],
                "mu": s["mu"],
                "n": s["n"],
            })
    if rows:
        with open(summary_path, "w", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=["param", "outcome", "mu_star",
                               "mu_star_ci_lo", "mu_star_ci_hi",
                               "sigma", "mu", "n"])
            writer.writeheader()
            for row in rows:
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=None,
                        help="Random seed for trajectory generation and sims")
