...
python3 -u batch/batch.py -n "$N_RUNS" --skin normal --study tumor --validate 2>&1 | tee "$RESULTS/log_tumor.txt"
...
python3 -u batch/batch.py -n "$N_RUNS" --skin normal --study tumor-wound --validate 2>&1 | tee "$RESULTS/log_tumor_wound.txt"
LATEST_TW=$(ls -td batch/results/consensus_normal_tumor-wound_* 2>/dev/null | head -1)
if [ -n "$LATEST_TW" ]; then
...
python3 -u batch/batch.py -n "$N_RUNS" --skin aged --study tumor --validate 2>&1 | tee "$RESULTS/log_aged_tumor.txt"
