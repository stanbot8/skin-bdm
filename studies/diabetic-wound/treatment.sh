...
#
# Usage:
#   ./studies/diabetic-wound/treatment.sh                        # all 8 treatments (~3-4h)
#   ./studies/diabetic-wound/treatment.sh --quick                # baseline + 2 (~45min)
#   ./studies/diabetic-wound/treatment.sh --treatments=hbo,npwt  # pick specific ones
#   ./studies/diabetic-wound/treatment.sh --combos=pairs         # singles + all pairwise combos
#   ./studies/diabetic-wound/treatment.sh --combos=triples       # singles + 2- and 3-combos
#   ./studies/diabetic-wound/treatment.sh --combos=all           # singles + all 2^N-1 subsets
#   ./studies/diabetic-wound/treatment.sh --no-excel             # skip Excel generation
#
# Output lands in studies/diabetic-wound/results/
...
python3 -u scripts/study/treatment_study.py --treatments="${TREATMENTS}" ${COMBO_FLAG} ${WORKERS_FLAG}
elapsed=$(( SECONDS - t0 ))
mins=$(( elapsed / 60 ))
secs=$(( elapsed % 60 ))
echo ""
...
# --- Generate Excel workbook (from output, not copied to study dir) ---
SRC="output/treatment_study"
if $EXCEL; then
  if python3 -c "import openpyxl" 2>/dev/null; then
    echo ""
    echo "Generating Excel workbook..."
    python3 scripts/study/generate_study.py "$SRC" -o "$SRC" --name "diabetic_study" 2>&1 || true
  else
    echo ""
...
if [ -f "$SRC/metrics_baseline.csv" ]; then
  echo ""
  echo "Generating baseline plots..."
  PLOT_DIR="$SRC/plots"
  mkdir -p "$PLOT_DIR"
  python3 scripts/analysis/plot_metrics.py "$SRC/metrics_baseline.csv" -o "$PLOT_DIR" 2>&1 || true
fi
...
if [ -f scripts/analysis/resilience_analysis.py ]; then
  ...
  python3 scripts/analysis/resilience_analysis.py "$SRC" 2>&1 || true
fi

# --- Summary ---
n_csv=$(ls "$SRC"/*.csv 2>/dev/null | wc -l)
n_xlsx=$(ls "$SRC"/*.xlsx 2>/dev/null | wc -l)
n_plots=$(ls "$SRC"/plots/*.png 2>/dev/null | wc -l)

echo ""
echo "============================================"
echo "  Study complete"
echo "============================================"
echo "  Results in: output/treatment_study/"
echo "  CSVs:  ${n_csv}"
echo "  Excel: ${n_xlsx}"
echo "  Plots: ${n_plots}"
echo ""
