...
#
# Usage:
#   ./studies/wound/skin-comparison.sh           # all profiles (10-run consensus)
#   ./studies/wound/skin-comparison.sh --quick   # 5-run consensus
#   ./studies/wound/skin-comparison.sh -n 20     # 20-run consensus
#
# Output lands in studies/wound/results/
...
python3 -u batch/batch.py -n "$N_RUNS" --skin normal --study wound --validate 2>&1 | tee "$RESULTS/log_normal.txt"
...
python3 -u batch/batch.py -n "$N_RUNS" --skin aged --study wound --validate 2>&1 | tee "$RESULTS/log_aged.txt"
...
python3 -u batch/batch.py -n "$N_RUNS" --skin diabetic --study diabetic --validate 2>&1 | tee "$RESULTS/log_diabetic.txt"
LATEST=$(ls -td batch/results/consensus_diabetic_diabetic-wound_* 2>/dev/null | head -1)
if [ -n "$LATEST" ]; then
...
echo "============================================"
echo "  Results in: studies/wound/results/"
echo "  CSVs:  ${n_csv}"
echo "  Plots: ${n_plots}"
echo ""
